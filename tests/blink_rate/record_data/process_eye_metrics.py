...
def process_eye(frame, timestamp_dt, draw_mesh=False, draw_contours=False, show_axis=False, draw_eye=False, verbose=0):
    ...
    face_detected, left_eye, right_eye, normalised_eye_speed, yaw, pitch, roll, diagnostic_frame = face_processor.process_face(
        frame, draw_mesh=draw_mesh, draw_contours=draw_contours, show_axis=show_axis, draw_eye=draw_eye)

    focus = False

    if face_detected == 0 or (left_eye is None and right_eye is None):
        return face_detected, None, None, None, None, None, None, None, None, focus, None, None, "None", diagnostic_frame
    ...
    if (normalised_eye_speed > 0.25 or (abs(yaw) > 25 or abs(pitch) > 30)):
        return face_detected, normalised_eye_speed, yaw, pitch, roll, avg_ear, blink_detected, None, None, focus, None, None, "None", diagnostic_frame
    ...
    # Ensure all resources are released after processing
    # If you open files in IrisProcessor, make sure to close them after processing
